/**
 * Freedom World — PostHog Event Name Constants
 * Sprint 6.1 — No magic strings. Every event fired in the app must use these.
 */

export const EVENTS = {
  ONBOARDING_STARTED: 'onboarding_started',
  Q1_ANSWERED: 'q1_answered',
  Q2_SCRAPE_SUCCESS: 'q2_scrape_success',
  Q2_SCRAPE_SKIP: 'q2_scrape_skip',
  Q2_SCRAPE_FAILED: 'q2_scrape_failed',
  Q3_MOOD_SELECTED: 'q3_mood_selected',
  Q4_COLOR_SELECTED: 'q4_color_selected',
  Q4B_UI_STYLE_SELECTED: 'q4b_ui_style_selected',
  FIRST_PREVIEW_SHOWN: 'first_preview_shown',
  SIGNUP_WALL_SHOWN: 'signup_wall_shown',
  SIGNUP_COMPLETED: 'signup_completed',
  SIGNUP_ABANDONED: 'signup_abandoned',
  COMMUNITY_CREATED: 'community_created',
  APP_BUILD_STARTED: 'app_build_started',
  APP_BUILD_COMPLETED: 'app_build_completed',
  APP_BUILD_FAILED: 'app_build_failed',
  APP_DEPLOYED: 'app_deployed',
  DEPLOY_FAILED: 'deploy_failed',
  ITERATION_STARTED: 'iteration_started',
  ITERATION_COMPLETED: 'iteration_completed',
  TOKEN_LIMIT_REACHED: 'token_limit_reached',
  SESSION_ABANDONED: 'session_abandoned',
  AD_HOC_REQUEST: 'ad_hoc_request',
  FEATURES_SELECTED: 'features_selected',
  CHAT_MESSAGE_SENT: 'chat_message_sent',
  CHAT_MESSAGE_RECEIVED: 'chat_message_received',
  TAG_EXTRACTED: 'tag_extracted',
  Q5_PRODUCTS_CAPTURED: 'q5_products_captured',
  Q6_PRIORITIES_SET: 'q6_priorities_set',
  Q7_ANTI_PREFS_SET: 'q7_anti_prefs_set',
  Q8_AUDIENCE_SET: 'q8_audience_set',
  Q9_FEATURES_SELECTED: 'q9_features_selected',
  Q10_REVIEW_COMPLETE: 'q10_review_complete',
  PREVIEW_OPENED: 'preview_opened',
  PREVIEW_CLOSED: 'preview_closed',
  IMAGE_UPLOAD_STARTED: 'image_upload_started',
  IMAGE_UPLOAD_COMPLETED: 'image_upload_completed',
  IMAGE_UPLOAD_FAILED: 'image_upload_failed',
  COLOR_PICKER_SHOWN: 'color_picker_shown',
  COLOR_SELECTED: 'color_selected',
  STYLE_PICKER_SHOWN: 'style_picker_shown',
  STYLE_SELECTED: 'style_selected',
  GO_LIVE_CLICKED: 'go_live_clicked',
  BUILD_STEP_REACHED: 'build_step_reached',
  APP_URL_CLICKED: 'app_url_clicked',
  SPEC_COMPLETENESS: 'spec_completeness',
  AB_TEST_ASSIGNED: 'ab_test_assigned',
  AB_TEST_CONVERSION: 'ab_test_conversion',
} as const;

export type EventName = (typeof EVENTS)[keyof typeof EVENTS];
